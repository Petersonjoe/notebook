...
def searchURL(url: str='') -> str:
    ...
    if (_response_str is None) or (len(_response_str)==0):
        logging.error(f"NO response from the url query! <The url: {url} is invalid, no data returned.>")
        return None
...
class CompanySpider(object):
    ...
    def __init__(self, company_name: str='') -> None:
        ...
        """
        if company_name == '':
            logging.error('Wrong call function, please specify the company name.')
    ...
    def PID(self) -> int:
        ...
        """
        if self.__pid__ is None:
            return -99999999999999
        else:
            return int(self.__pid__)
    ...
    def __unionRisks__(self) -> tuple:
        ...
        if ( 'total' not in response_dict['data'].keys() ) or \
            ...
            logging.warning(f'No judgement docs found for {self._company_name}.')
    ...
    def getJudgementDocs(self) -> pd.DataFrame:
        ...
        for doc in judgement_doc_list:
            ...
            if _count > JUDGE_DOC_MAX:
                logging.warning('NUM of judgement doc details exceeds the tolerance 20.')
                break
    ...
    def getOpenNoticeDetails(self) -> pd.DataFrame:
        ...
        logging.info(f'Total open notice number is {total_notice_num}.')
        _count = 0
        for notice in open_notice_list:
            ...
            _count += 1
            if _count > JUDGE_DOC_MAX:
                logging.warning("NUM of open notice details exceeds the tolerance 20.")
                break
